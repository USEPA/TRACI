...
class LifeCycleStageForm(ModelForm):
    """
    Life Cycle Stages cross-reference with product, constitutes a
    ...
    name = ModelChoiceField(
        queryset=LifeCycleStageName.objects.all(),
        initial=0,
        required=True,
...
class ProcessForm(ModelForm):
    ...
    location = ModelChoiceField(
        queryset=Location.objects.filter(geogid__lte=61).order_by('geogid'),
        initial=0,
        required=True,
        label=_("Process Location (Parent - Required)"),
        widget=Select(attrs={'class': 'form-control mb-2'}))
    # The child location will be used to narrow down past states or regions.
    child_location = ModelChoiceField(
        queryset=Location.objects.none(),
        initial=0,
        required=False,
        label=_("Process Location (Child - Optional)"),
        widget=Select(attrs={'class': 'form-control mb-2'}))
    lifecyclestage = ModelChoiceField(
        queryset=LifeCycleStage.objects.all(),
        initial=0,
        required=True,
        label=_("Parent Life Cycle Stage"),
        widget=TextInput(attrs={
            'class': 'form-control mb-2',
            'readonly': 'readonly'
        }))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['name'].label_from_instance = lambda obj: "%s" % obj.name
        self.fields[
            'location'].label_from_instance = lambda obj: "%s" % obj.name
...
class SubstanceTypeForm(ModelForm):
    ...
    """
    substance_type = ModelChoiceField(
...
class ResourceForm(ModelForm):
    ...
    substance_type = ModelChoiceField(
        ...
        label=_("Name"),
        widget=Select(attrs={'class': 'form-control mb-2'}))
    resource_media = ModelChoiceField(
        queryset=Media.objects.filter(name='No Media'),
        initial=0,
        required=True,
        label=_("Media"),
        widget=Select(attrs={'class': 'form-control mb-2'}))
    resource_quantity = FloatField(
        label=_("Quantity"),
        widget=NumberInput(attrs={'class': 'form-control mb-2'}),
        required=True,
        initial=0)
    resource_unit = ModelChoiceField(
        queryset=Unit.objects.filter(chemical=False),
        initial=0,
        required=True,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            'resource_media'].label_from_instance = lambda obj: "%s" % obj.name
        self.fields[
            'resource_unit'].label_from_instance = lambda obj: "%s" % obj.name

    class Meta:
        """Meta data for Resource form."""

        model = Resource
        fields = ('substance_type', 'resource_media', 'resource_quantity',
                  'resource_unit', 'process')
...
#     """
#    Chemical Resources/Releases store the quantity and unit information
#    pertaining to certain chemicals that are either required inputs
#    for a process, or the outputs of a process.
#    """
...
#         required=True,
#         initial=0)
#     # Only pull chemical units first, then if the user selects an input type,
#     # then get the appropriate unit options
#     unit = ModelChoiceField(
#         queryset=Unit.objects.filter(chemical_unit=True),
#         initial=0,
#         required=True,
#         label=_("Units of Measurement"),
#         widget=Select(attrs={'class': 'form-control mb-2'}))
...
#         self.fields['type'].label_from_instance = lambda obj: "%s (%s)" % (
#             obj.name, obj.type)
