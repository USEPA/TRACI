...
var Table = (function(){
  ...
  table.filter = function(o,filters,args) {
    ...
    if (!filters) {
      ...
      for (var i=0,L=filters.length; i<L; i++) {
        ...
        // and assign it's column index to the filter if it does not have one. This way,
  ...
  */
  table.scrape = function(o,args) {
    ...
    var page = tdata.page;
    if (def(page)) {
      // do not let the page go before the beginning
      ...
      for (var j=0,L2=tb.rows.length; j<L2; j++) {
        ...
        if (!hideRow) {
          ...
          if (def(page)) {
            ...
            // the last page and we need to back up. do not want to filter again!
