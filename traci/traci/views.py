...
from traci.settings import CITE_SCRIPT, DOWNLOADS_DIR, MANUAL_NAME, \
    EXCEL_TOOL_NAME
...
def contact(request):
    ...
    link = 'https://www.epa.gov/chemical-research/tool-reduction-and-' + \
        'assessment-chemicals-and-other-environmental-impacts-traci'
    link_label = 'TRACI'
    return render(
...
def about(request):
    ...
    return render(
        request, 'main/about.html', {
            ...
            'message':
            'TRACI allows the examination of the potential for impacts ' +
            'associated with fossil fuel use and chemical releases ' +
            'resulting from the processes involved in producing a product. ' +
            'TRACI allows the user to examine the potential for impacts ' +
            'for a single life cycle stage, or the whole life cycle, ' +
            'and to compare the results among products or processes.',
            'year':
            datetime.now().year,
        })
...
class DashboardView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        ctx = {
            'open_tab': request.GET.get('show', 'teams'),
            'user': request.user
        }
        ...
        headers = {'X-CSRFToken': token, 'Cookie': request.META["HTTP_COOKIE"]}

        # get the teams of which this user is a member
        teams_data = requests.get(api_domain + '/teams/api/team/',
                                  headers=headers)
...
def download_file(_response, name):
    ...
    if ext == 'pdf':
        ...
        with open(file, 'rb') as xls:
            response = HttpResponse(xls, content_type=con_type)
...
def download_manual(request):
    """Begins downloading TRACI user manual stored on the server."""
    return download_file(request, MANUAL_NAME)


@login_required
def download_excel_tool(request):
    """Begins downloading TRACI Excel Tool stored on the server."""
